...
const form = useForm({
    name: props.page_data.menu.name,
    route_name: props.page_data.menu.route_name || '',
    url: props.page_data.menu.url || '',
    icon: props.page_data.menu.icon || '',
    parent_id: props.page_data.menu.parent_id,
    order: props.page_data.menu.order,
    permission_name: props.page_data.menu.permission_name || '',
    is_active: props.page_data.menu.is_active,
});
...
// Flatten menus for parent selection, excluding current menu and its children
const getChildIds = (menu: Menu): number[] => {
    const ids = [menu.id];
    if (menu.children) {
...
const excludeIds = computed(() => getChildIds(props.page_data.menu));

const flattenMenusForSelect = (menus: Menu[], depth: number = 0): ComboboxOption[] => {
    const result: ComboboxOption[] = [];
    for (const menu of menus) {
        if (excludeIds.value.includes(menu.id)) continue;
        ...
        <div class="flex h-full flex-1 flex-col gap-4 p-4">
            ...
            <form @submit.prevent="submit" class="max-w-2xl space-y-6">
                ...
                <div class="flex justify-end gap-4">
                    ...
                    <Button type="submit" :disabled="form.processing"> Update Menu </Button>
                </div>
            </form>
        </div>
    </AppLayout>
