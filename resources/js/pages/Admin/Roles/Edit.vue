...
import type { BreadcrumbItem, Permission, PermissionGroup, Role } from '@/types';
...
const form = useForm({
    name: props.page_data.role.name,
    permissions: [...props.page_data.rolePermissions],
});
...
const isSuperAdmin = props.page_data.role.name === 'super-admin';
</script>
    ...
    <AppLayout :breadcrumbs="page_setting.breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 p-4">
            <div class="flex items-center gap-4">
                ...
                <h1 class="text-2xl font-bold">Edit Role: {{ page_data.role.name }}</h1>
            </div>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div class="max-w-md space-y-2">
                            ...
                            />
                            <p v-if="isSuperAdmin" class="text-sm text-muted-foreground">The super-admin role name cannot be changed.</p>
                    ...
                    <CardHeader>
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <CardDescription>
                                    {{ isSuperAdmin ? 'Super admin has all permissions by default' : 'Select permissions for this role' }}
                                </CardDescription>
                            </div>
                    ...
                    <CardContent class="space-y-6">
                        <div v-for="group in page_data.permissionGroups" :key="group.id" class="space-y-3">
                            <div class="flex items-center gap-2 border-b pb-2">
                                <Checkbox
                                    :id="`group-${group.id}`"
                                    :checked="isSuperAdmin || isGroupAllSelected(group.permissions)"
                                    :disabled="isSuperAdmin"
                            ...
                            <div class="ml-6 grid gap-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                                <div v-for="permission in group.permissions" :key="permission.id" class="flex items-center gap-2">
                                    <Checkbox
                                        ...
                                        :checked="isSuperAdmin || isPermissionSelected(permission.id)"
                                        :disabled="isSuperAdmin"
